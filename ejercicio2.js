...
function ferraris (arg) {
    return arg.filter((x) => x.brand == "Ferrari")
}

function nineties (arg) {
    return arg.filter(x=> x.model > 1980 && x.model < 1990)
}

function list(arg) {
    return arg.map((x) => `This ${x.brand} from ${x.model} costs $${x.price}`)
}

function bmwSum(arg) {
    return arg.filter((x) => x.brand == "BMW").map(x => x.price).reduce((a,b) => a + b)
}
